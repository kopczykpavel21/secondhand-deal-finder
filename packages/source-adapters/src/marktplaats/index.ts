...
const API_URL = 'https://www.marktplaats.nl/lrp/api/search';
...
function extractPriceFromLabel(label: string): number | null {
  const cleaned = label.replace(/[^\d,.]/g, '').replace(',', '.');
  const parsed = parseFloat(cleaned);
  return Number.isFinite(parsed) && parsed > 0 ? parsed : null;
}

function parseMarktplaatsDate(raw: string | null | undefined, safeDate: (s: string | null | undefined) => Date | null): Date | null {
  if (!raw) return null;
...
export class MarktplaatsAdapter extends BaseAdapter {
  ...
  constructor(config: Partial<AdapterConfig> = {}) {
    super({ timeout: 15_000, rateLimitMs: 1_500, retries: 2, ...config }, getMarketConfig('nl'));
  }

  async searchListings(query: string, filters?: SearchFilters): Promise<NormalizedListing[]> {
    return this.withRetry(() => this.fetchListings(query, filters), 'marktplaats.search');
  }
  ...
  private async fetchViaPage(query: string, filters?: SearchFilters): Promise<NormalizedListing[]> {
    try {
      ...
      if (!match) {
        this.log('No __NEXT_DATA__ found in Marktplaats page');
        return [];
      }
    ...
    for (const listing of listings) {
      ...
      if (priceInfo) {
        ...
        if (priceCents != null) {
          ...
          if (priceLabel) price = extractPriceFromLabel(priceLabel);
        }
      }
      ...
      const postedAt = parseMarktplaatsDate(rawDate, (s) => this.safeDate(s));
