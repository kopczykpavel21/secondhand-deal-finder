...
function formatDate(date: Date | string | null): string {
  ...
  if (hours < 1) return 'Gerade eingestellt';
  if (hours < 24) return `Vor ${Math.floor(hours)} Std.`;
    ...
    <article className={clsx(
      ...
      {likelySold && (
        <div className="bg-amber-50 border-b border-amber-200 px-3 py-1.5 text-xs text-amber-700 flex items-center gap-1.5">
          ...
          Wahrscheinlich verkauft — Inserat ist älter als 3 Wochen
        </div>
      )}
        ...
        <div className="flex-1 p-3 sm:p-4 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0 flex-1">
              ...
              {listing.description ? (
                ...
                <p className="mt-1 text-xs text-slate-400 italic">Beschreibung nicht verfügbar.</p>
              ) : descState === 'loading' ? (
          ...
          {debugMode && (
            <div className="mt-2">
              ...
              >
                {showDebug ? 'Score-Detail ausblenden' : 'Score-Detail'}
              </button>
              {showDebug && (
...
function ScoreBreakdown({ components }: { components: ScoredListing['scoreComponents'] }) {
  const rows: Array<[string, number]> = [
    ['Relevanz (Übereinstimmung)', components.relevance],
    ['Preis-Leistung', components.valueForMoney],
    ['Zustand', components.condition],
    ['Aktualität', components.freshness],
    ['Vollständigkeit', components.completeness],
    ['Verkäufer-Vertrauen', components.sellerTrust],
    ['Käufer-Interesse', components.engagement],
    ['Malus: Sponsored', components.promotedPenalty],
    ['Malus: Spam', components.spamPenalty],
  ];
  return (
    <>
      {rows.map(([label, val]) => (
