...
const inlineCoordinator = createDutchSearchCoordinator({ cache: searchCache });
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const cached = await searchCache.get(createSearchCacheKey(searchRequest, 50, 100, 'nl'));
    if (cached) {
    ...
    const job = await enqueueSearchJob('nl', searchRequest);
    if (!job) {
      const result = await inlineCoordinator.search(searchRequest);
      return NextResponse.json(result, {
        headers: {
