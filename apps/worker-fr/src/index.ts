...
async function processJob(job: SearchJobPayload): Promise<void> {
  const coordinator = createProductionFrenchSearchCoordinator(getSearchCache());
  try {
    ...
    console.error(`[worker-fr] job ${job.jobId} failed:`, error);
    await markSearchJobFailed(job, (error as Error).message);
  }
}

async function run(): Promise<void> {
  console.log(`[worker-fr] starting with concurrency=${workerConcurrency}`);
  const active = new Set<Promise<void>>();
  while (!shuttingDown) {
    while (active.size < workerConcurrency) {
      const job = await claimSearchJob('fr');
      if (!job) break;
...
run().catch((error) => { console.error('[worker-fr] fatal error:', error); process.exitCode = 1; });
