...
import {
  ...
  claimSearchJob,
  createProductionGreeceSearchCoordinator,
  getSearchCache,
  markSearchJobFailed,
...
async function processJob(job: SearchJobPayload): Promise<void> {
  const coordinator = createProductionGreeceSearchCoordinator(getSearchCache());
  ...
  try {
    ...
    console.error(`[worker-gr] job ${job.jobId} failed:`, error);
    await markSearchJobFailed(job, message);
  }
}

async function run(): Promise<void> {
  console.log(`[worker-gr] starting with concurrency=${workerConcurrency}`);
  const active = new Set<Promise<void>>();

  while (!shuttingDown) {
    while (active.size < workerConcurrency) {
      const job = await claimSearchJob('gr');
      if (!job) break;
...
run().catch((error) => {
  console.error('[worker-gr] fatal error:', error);
  process.exitCode = 1;
});
