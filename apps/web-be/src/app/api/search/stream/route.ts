...
const inlineCoordinator = createBelgianSearchCoordinator({ cache: searchCache });
...
export async function GET(req: NextRequest) {
  ...
  const cached = await searchCache.get(createSearchCacheKey(searchRequest, 50, 100, 'be'));
  if (cached) {
  ...
  const job = await enqueueSearchJob('be', searchRequest);
  if (!job) {
    const stream = new ReadableStream({
      async start(controller) {
        try {
