...
const inlineCoordinator = createBritishSearchCoordinator({ cache: searchCache });
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const cached = await searchCache.get(createSearchCacheKey(searchRequest, 50, 100, 'gb'));
    if (cached) {
    ...
    const job = await enqueueSearchJob('gb', searchRequest);
    if (!job) {
      const result = await inlineCoordinator.search(searchRequest);
      return NextResponse.json(result, {
        headers: {
