...
const inlineCoordinator = createRomanianSearchCoordinator({ cache: searchCache });
...
export async function GET(req: NextRequest) {
  ...
  const cached = await searchCache.get(createSearchCacheKey(searchRequest, 50, 100, 'ro'));
  if (cached) {
  ...
  const job = await enqueueSearchJob('ro', searchRequest);
  if (!job) {
    const stream = new ReadableStream({
      async start(controller) {
        try {
