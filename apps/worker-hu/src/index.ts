...
import { appendSearchJobEvent, claimSearchJob, createProductionHungarianSearchCoordinator, getSearchCache, markSearchJobFailed, storeSearchJobResult, type SearchJobPayload } from '@sdf/platform';
...
async function processJob(job: SearchJobPayload): Promise<void> {
  const coordinator = createProductionHungarianSearchCoordinator(getSearchCache());
  try {
  ...
  } catch (error) { console.error('[worker-hu] job ' + (job.jobId) + ' failed:', error); await markSearchJobFailed(job, (error as Error).message); }
}
async function run(): Promise<void> {
  console.log('[worker-hu] starting with concurrency=' + workerConcurrency);
  const active = new Set<Promise<void>>();
  while (!shuttingDown) {
    while (active.size < workerConcurrency) { const job = await claimSearchJob('hu'); if (!job) break; const task = processJob(job).finally(() => active.delete(task)); active.add(task); }
...
run().catch((error) => { console.error('[worker-hu] fatal error:', error); process.exitCode = 1; });
