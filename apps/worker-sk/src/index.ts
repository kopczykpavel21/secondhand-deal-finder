...
async function processJob(job: SearchJobPayload): Promise<void> {
  const coordinator = createProductionSlovakSearchCoordinator(getSearchCache());
  ...
  try {
    ...
    console.error(`[worker-sk] job ${job.jobId} failed:`, error);
    await markSearchJobFailed(job, message);
  }
}

async function run(): Promise<void> {
  console.log(`[worker-sk] starting with concurrency=${workerConcurrency}`);
  const active = new Set<Promise<void>>();

  while (!shuttingDown) {
    while (active.size < workerConcurrency) {
      const job = await claimSearchJob('sk');
      if (!job) break;
...
run().catch((error) => {
  console.error('[worker-sk] fatal error:', error);
  process.exitCode = 1;
});
