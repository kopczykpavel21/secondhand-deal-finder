...
async function processJob(job: SearchJobPayload): Promise<void> {
  const coordinator = createProductionCzechSearchCoordinator(getSearchCache());
  ...
  try {
    ...
    console.error(`[worker-cz] job ${job.jobId} failed:`, error);
    await markSearchJobFailed(job, message);
  }
}

async function run(): Promise<void> {
  console.log(`[worker-cz] starting with concurrency=${workerConcurrency}`);
  const active = new Set<Promise<void>>();

  while (!shuttingDown) {
    while (active.size < workerConcurrency) {
      const job = await claimSearchJob('cz');
      if (!job) break;
...
run().catch((error) => {
  console.error('[worker-cz] fatal error:', error);
  process.exitCode = 1;
});
