...
const inlineCoordinator = createSwedishSearchCoordinator({ cache: searchCache });
...
export async function GET(req: NextRequest) {
  ...
  const cached = await searchCache.get(createSearchCacheKey(searchRequest, 50, 100, 'se'));
  if (cached) {
  ...
  const job = await enqueueSearchJob('se', searchRequest);
  if (!job) {
    const stream = new ReadableStream({
      async start(controller) {
        try {
