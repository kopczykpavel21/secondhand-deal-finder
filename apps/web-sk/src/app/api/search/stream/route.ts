...
const inlineCoordinator = createSlovakSearchCoordinator({ cache: searchCache });
...
export async function GET(req: NextRequest) {
  ...
  const cached = await searchCache.get(createSearchCacheKey(searchRequest, 50, 100, 'sk'));
  if (cached) {
  ...
  const job = await enqueueSearchJob('sk', searchRequest);
  if (!job) {
    const stream = new ReadableStream({
      async start(controller) {
        try {
