...
async function processJob(job: SearchJobPayload): Promise<void> {
  const coordinator = createProductionGermanSearchCoordinator(getSearchCache());
  ...
  try {
    ...
    console.error(`[worker-de] job ${job.jobId} failed:`, error);
    await markSearchJobFailed(job, message);
  }
}

async function run(): Promise<void> {
  console.log(`[worker-de] starting with concurrency=${workerConcurrency}`);
  const active = new Set<Promise<void>>();

  while (!shuttingDown) {
    while (active.size < workerConcurrency) {
      const job = await claimSearchJob('de');
      if (!job) break;
...
run().catch((error) => {
  console.error('[worker-de] fatal error:', error);
  process.exitCode = 1;
});
