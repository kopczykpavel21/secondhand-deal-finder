...
async function processJob(job: SearchJobPayload): Promise<void> {
  const coordinator = createProductionRomanianSearchCoordinator(getSearchCache());
  ...
  try {
    ...
    console.error(`[worker-ro] job ${job.jobId} failed:`, error);
    await markSearchJobFailed(job, message);
  }
}

async function run(): Promise<void> {
  console.log(`[worker-ro] starting with concurrency=${workerConcurrency}`);
  const active = new Set<Promise<void>>();

  while (!shuttingDown) {
    while (active.size < workerConcurrency) {
      const job = await claimSearchJob('ro');
      if (!job) break;
...
run().catch((error) => {
  console.error('[worker-ro] fatal error:', error);
  process.exitCode = 1;
});
