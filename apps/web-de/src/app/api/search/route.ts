...
const inlineCoordinator = createGermanSearchCoordinator({ cache: searchCache });
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const cached = await searchCache.get(createSearchCacheKey(searchRequest, 50, 100, 'de'));
    if (cached) {
    ...
    const job = await enqueueSearchJob('de', searchRequest);
    if (!job) {
      const result = await inlineCoordinator.search(searchRequest);
      return NextResponse.json(result, {
        headers: {
    ...
    console.error('[web-de/api/search] Unhandled error:', err);
