...
import { appendSearchJobEvent, claimSearchJob, createProductionItalianSearchCoordinator, getSearchCache, markSearchJobFailed, storeSearchJobResult, type SearchJobPayload } from '@sdf/platform';
...
async function processJob(job: SearchJobPayload): Promise<void> {
  const coordinator = createProductionItalianSearchCoordinator(getSearchCache());
  try {
  ...
  } catch (error) { console.error('[worker-it] job ' + (job.jobId) + ' failed:', error); await markSearchJobFailed(job, (error as Error).message); }
}
async function run(): Promise<void> {
  console.log('[worker-it] starting with concurrency=' + workerConcurrency);
  const active = new Set<Promise<void>>();
  while (!shuttingDown) {
    while (active.size < workerConcurrency) { const job = await claimSearchJob('it'); if (!job) break; const task = processJob(job).finally(() => active.delete(task)); active.add(task); }
...
run().catch((error) => { console.error('[worker-it] fatal error:', error); process.exitCode = 1; });
