...
const inlineCoordinator = createItalianSearchCoordinator({ cache: searchCache });
...
export async function GET(req: NextRequest) {
  ...
  const cached = await searchCache.get(createSearchCacheKey(searchRequest, 50, 100, 'it'));
  if (cached) {
  ...
  const job = await enqueueSearchJob('it', searchRequest);
  if (!job) {
    const stream = new ReadableStream({
      async start(controller) {
        try {
