...
async function processJob(job: SearchJobPayload): Promise<void> {
  const coordinator = createProductionPolishSearchCoordinator(getSearchCache());
  ...
  try {
    ...
    console.error(`[worker-pl] job ${job.jobId} failed:`, error);
    await markSearchJobFailed(job, message);
  }
}

async function run(): Promise<void> {
  console.log(`[worker-pl] starting with concurrency=${workerConcurrency}`);
  const active = new Set<Promise<void>>();

  while (!shuttingDown) {
    while (active.size < workerConcurrency) {
      const job = await claimSearchJob('pl');
      if (!job) break;
...
run().catch((error) => {
  console.error('[worker-pl] fatal error:', error);
  process.exitCode = 1;
});
