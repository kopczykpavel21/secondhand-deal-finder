...
const inlineCoordinator = createFrenchSearchCoordinator({ cache: searchCache });
...
export async function GET(req: NextRequest) {
  ...
  const cached = await searchCache.get(createSearchCacheKey(searchRequest, 50, 100, 'fr'));
  if (cached) {
  ...
  const job = await enqueueSearchJob('fr', searchRequest);
  if (!job) {
    const stream = new ReadableStream({
      async start(controller) {
        try {
