...
const inlineCoordinator = createGreeceSearchCoordinator({ cache: searchCache });
...
export async function GET(req: NextRequest) {
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { error: 'Μη έγκυρο αίτημα', issues: parsed.error.flatten() },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const cached = await searchCache.get(createSearchCacheKey(searchRequest, 50, 100, 'gr'));
    if (cached) {
    ...
    const job = await enqueueSearchJob('gr', searchRequest);
    if (!job) {
      const result = await inlineCoordinator.search(searchRequest);
      return NextResponse.json(result, {
        headers: {
    ...
    console.error('[web-gr/api/search] Unhandled error:', err);
    return NextResponse.json({ error: 'Η αναζήτηση απέτυχε' }, { status: 500 });
  }
}
