...
const DISMISSED_KEY = 'sdf-gr-dismissed';
      ...
      <header className="pt-12 pb-8 px-4">
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 bg-brand-100 text-brand-700 text-xs font-semibold px-3 py-1 rounded-full mb-4">
            ...
            Beta · Vinted · Facebook · Shpock
          </div>
          <h1 className="text-3xl sm:text-4xl font-bold text-slate-900 tracking-tight">
            Secondhand Finder Ελλάδα
          </h1>
          <p className="mt-2 text-slate-500 text-base sm:text-lg max-w-md mx-auto">
            Βρες τις καλύτερες προσφορές σε Vinted, Facebook και Shpock – ταξινομημένες κατά πραγματική αξία.
          </p>
        </div>
        ...
        <div className="max-w-2xl mx-auto space-y-3">
          ...
          >
            {debugMode ? 'Απόκρυψη βαθμολογίας' : 'Εμφάνιση βαθμολογίας'}
          </button>
        </div>
      </header>
      ...
      <main className="max-w-2xl mx-auto px-4 pb-16">
            ...
            <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
              <div className="flex items-center gap-2">
                <div className="text-sm text-slate-500">
                  ...
                  {' '}από {total} αποτελέσματα · „{currentQuery}"
                  {isSuccess && (
                  ...
                  {marketMedian != null && (
                    <span className="ml-2 text-xs bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">
                      Τιμή αγοράς ~{new Intl.NumberFormat('el-GR', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(marketMedian)}
                    </span>
                  )}
                </div>
                {isLoading && (
            ...
            {(hasPrevPage || hasNextPage) && (
              <div className="mt-6 flex items-center justify-center gap-3">
                ...
                <span className="text-sm text-slate-400">
                  Σελίδα {page + 1} από {Math.ceil(allResults.length / PAGE_SIZE)}
                </span>
                {hasNextPage && (
                  <button
                    onClick={() => { setPage((p) => p + 1); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
        ...
        {state.status === 'error' && (
          <div className="bg-red-50 border border-red-200 rounded-2xl p-6 text-center">
            <p className="font-semibold text-red-700">Η αναζήτηση απέτυχε</p>
        ...
        {noResults && (
          <div className="bg-white border border-slate-200 rounded-2xl p-10 text-center">
            ...
            <p className="font-semibold text-slate-700">Δεν βρέθηκαν αποτελέσματα</p>
            <p className="text-slate-400 text-sm mt-1">Δοκίμασε διαφορετικές λέξεις-κλειδιά ή άλλαξε τα φίλτρα.</p>
          </div>
        )}
        ...
        {state.status === 'idle' && (
          <div className="text-center mt-16 text-slate-400 space-y-3">
            ...
            <p className="font-medium text-slate-400">Τι ψάχνεις;</p>
            <p className="text-sm text-slate-300">Ψάχνουμε σε Vinted, Facebook και Shpock ταυτόχρονα.</p>
          </div>
        )}
      </main>

      <footer className="text-center text-xs text-slate-400 pb-8">
        Secondhand Finder Ελλάδα · Beta · Δεδομένα από τρίτες υπηρεσίες, μόνο για ενημέρωση
      </footer>
    </div>
  );
}
