...
function formatPrice(price: number | null, currency: string): string {
  if (price === null) return 'Χωρίς τιμή';
...
function formatDate(date: Date | string | null): string {
  ...
  if (hours < 1) return 'Μόλις αναρτήθηκε';
  if (hours < 24) return `Πριν ${Math.floor(hours)} ώρ.`;
    ...
    <article className={clsx(
      ...
      {likelySold && (
        <div className="bg-amber-50 border-b border-amber-200 px-3 py-1.5 text-xs text-amber-700 flex items-center gap-1.5">
          ...
          Πιθανόν πωλήθηκε — Η αγγελία είναι παλαιότερη των 3 εβδομάδων
        </div>
      )}
        ...
        <div className="flex-1 p-3 sm:p-4 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0 flex-1">
              ...
              {listing.description ? (
                ...
                <p className="mt-1 text-xs text-slate-400 italic">Η περιγραφή δεν είναι διαθέσιμη.</p>
              ) : descState === 'loading' ? (
          ...
          {debugMode && (
            <div className="mt-2">
              ...
              >
                {showDebug ? 'Απόκρυψη λεπτομερειών βαθμολογίας' : 'Λεπτομέρειες βαθμολογίας'}
              </button>
              {showDebug && (
...
function ScoreBreakdown({ components }: { components: ScoredListing['scoreComponents'] }) {
  const rows: Array<[string, number]> = [
    ['Συνάφεια', components.relevance],
    ['Αξία για χρήμα', components.valueForMoney],
    ['Κατάσταση', components.condition],
    ['Επικαιρότητα', components.freshness],
    ['Πληρότητα', components.completeness],
    ['Αξιοπιστία πωλητή', components.sellerTrust],
    ['Ενδιαφέρον αγοραστών', components.engagement],
    ['Ποινή: Προωθούμενο', components.promotedPenalty],
    ['Ποινή: Spam', components.spamPenalty],
  ];
  return (
    <>
      {rows.map(([label, val]) => (
